...
import edu.upc.eetac.dsa.secretsites.dao.UserAlreadyExistsException;
...
public class UserResource {
    ...
    public Response registerUser(@FormParam("loginid") String loginid, @FormParam("password") String password, @FormParam("email") String email, @FormParam("fullname") String fullname, @Context UriInfo uriInfo) throws URISyntaxException {
        ...
        return Response.created(uri).type(SecretSitesMediaType.SECRETSITES_AUTH_TOKEN).entity(authenticationToken).build();
    }

    @Path("/{id}")
    @GET
    @Produces(SecretSitesMediaType.SECRETSITES_USER)
    ...
    @Path("/{id}")
    @PUT
    @Consumes(SecretSitesMediaType.SECRETSITES_USER)
    @Produces(SecretSitesMediaType.SECRETSITES_USER)
