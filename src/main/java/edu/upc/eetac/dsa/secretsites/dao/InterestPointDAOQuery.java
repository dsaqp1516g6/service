...
public interface InterestPointDAOQuery {
    public final static String CREATE_POINT = "insert into interestpoints (id, name, longitude, latitude, description) values (UNHEX(?), ?, ?, ?, ?)";
    public final static String GET_POINT_BY_ID = "select hex(id) as id, name, longitude, latitude, creation_timestamp, description from interestpoints where id=unhex(?)";
    public final static String GET_POINTS_BY_NAME = "select hex(id) as id, name, longitude, latitude, creation_timestamp, description from interestpoints where name LIKE ?";
    public final static String GET_POINTS = "select hex(id) as id, name, longitude, latitude, creation_timestamp, description from interestpoints";
    public final static String UPDATE_POINT = "update interestpoints set name=?, longitude=?, latitude=?, description=? where id=unhex(?) ";
    public final static String DELETE_POINT = "delete from interestpoints where id=unhex(?)";
    public final static String GET_POINTS_BY_LATLNG = "select hex(id) as id, name, longitude, latitude, creation_timestamp, description from interestpoints " +
    ...
    public final static String GET_POINTS_BY_STATUS = "select hex(i.id) as id, i.name, i.longitude, i.latitude, i.creation_timestamp, i.description from interestpoints i, user_points u where " +
